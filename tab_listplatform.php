...
?>
		...
		<div id="platformsPanel" style="border: 1px solid #000; background-color: #555555; padding: 15px; color: #FFFFFF;">
			...
			<h1 class="arcade" style="text-align: center;"><span style="color: #000;">Browse</span> <span style="color: #EF5F00;">Platforms</span></h1>
				...
				<form id="platformBrowseForm">
					<select name="stringPlatform" id="platformMenu" onchange="if($('#platformMenu').val() != 'select') { document.location.href = '<?= $baseurl ?>/browse/' + this.value + '/'; }" style="color: #333; width: 450px !important;">
	...
	<?php
		...
		*/
		...
		{	
			...
			//previous button
			if ($page > 1) 
				$pagination.= "<a href=\"$baseurl/browse/$stringPlatform/?sortBy=$sortBy&limit=$limit&page=$prev\">&laquo; prev</a>";
			else
			...
			{	
				for ($counter = 1; $counter <= $lastpage; $counter++)
				{
					if ($counter == $page)
						...
						$pagination.= "<a href=\"$baseurl/browse/$stringPlatform/?sortBy=$sortBy&limit=$limit&page=$counter\">$counter</a>";					
				}
			}
			elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
			{
				...
				if($page < 1 + ($adjacents * 2))		
				{
					...
					$pagination.= "...";
					$pagination.= "<a href=\"$baseurl/browse/$stringPlatform/?sortBy=$sortBy&limit=$limit&page=$lpm1\">$lpm1</a>";
					$pagination.= "<a href=\"$baseurl/browse/$stringPlatform/?sortBy=$sortBy&limit=$limit&page=$lastpage\">$lastpage</a>";		
				}
				//in middle; hide some front and some back
				elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
				{
					$pagination.= "<a href=\"$baseurl/browse/$stringPlatform/?sortBy=$sortBy&limit=$limit&page=1\">1</a>";
					$pagination.= "<a href=\"$baseurl/browse/$stringPlatform/?sortBy=$sortBy&limit=$limit&page=2\">2</a>";
					$pagination.= "...";
			...
			//next button
			if ($page < $counter - 1) 
				$pagination.= "<a href=\"$baseurl/browse/$stringPlatform/?sortBy=$sortBy&limit=$limit&page=$next\">next &raquo;</a>";
			else
	...
	<!-- Start Sort By -->
	<form style="text-align: right;" action="<?=  $baseurl ?>/browse/<?php if (isset($stringPlatform)) { echo "$stringPlatform/"; } ?>">
			...
			if ($function == 'Browse By Platform')  {
				...
				while ($game = mysql_fetch_object($result)) {
					...
					<tr>
						...
						<td class="<?php echo $class; ?>"><a href="<?php echo $baseurl; ?>/game/<?php echo $game->id; ?>/"><?php echo $game->GameTitle; ?></a></td>
						<td class="<?php echo $class; ?>"><img src="<?php echo $baseurl; ?>/images/common/consoles/png16/<?php echo $platformIdResult->icon; ?>" alt="<?php echo $platformIdResult->name; ?>" style="vertical-align: middle;" /> <?php echo $platformIdResult->name; ?></td>
						...
						<td align="center" class="<?php echo $class; ?>"><?php if($boxartResult != 0){ ?><img src="<?php echo $baseurl; ?>/images/common/icons/tick_16.png" alt="Yes" /><?php } else{ ?><img src="<?php echo $baseurl; ?>/images/common/icons/cross_16.png" alt="Yes" /><?php }?></td>
						<td align="center" class="<?php echo $class; ?>"><?php if($fanartResult != 0){ ?><img src="<?php echo $baseurl; ?>/images/common/icons/tick_16.png" alt="Yes" /><?php } else{ ?><img src="<?php echo $baseurl; ?>/images/common/icons/cross_16.png" alt="Yes" /><?php }?></td>
						<td align="center" class="<?php echo $class; ?>"><?php if($bannerResult != 0){ ?><img src="<?php echo $baseurl; ?>/images/common/icons/tick_16.png" alt="Yes" /><?php } else{ ?><img src="<?php echo $baseurl; ?>/images/common/icons/cross_16.png" alt="Yes" /><?php }?></td>
					</tr>
					<?php
					$gamecount++;
				}
